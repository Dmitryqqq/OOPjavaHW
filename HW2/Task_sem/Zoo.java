...
public class Zoo {
    ...
    // public void addAnimal (Fish newAnimal){  // не смог придумать как добавиьть рыбу
    //     this.animals.add(newAnimal);
    // }
    ...
    public Animal championBySwim (){
        List<Swimable> start = swimming();
        Swimable ferst = start.get(0);
        for (Swimable swimable : start) {
            if (swimable.swimSpeed()>ferst.swimSpeed()) {
                ferst = swimable;
            }
        }
